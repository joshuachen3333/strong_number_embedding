...
from run_stage2_harsh import nines_recall, CHI_TO_WLC_BOOK
...
def run_arm(label, conv_on, verses, model, verbose, samples):
    ...
    for (book_chi, book_eng, wlc_book, chap, sec, unv_sn) in verses:
        ...
        row = {"chap": chap, "sec": sec, "full_frac": sum(fp) / len(fp),
               ...
               "has_ylt": bool(ylt_verse_text(wlc_book, chap, sec))}
        rows.append(row)
        if verbose:
            r9 = f"{n9p}/{n9t}" if n9t else "—"
            print(f"  [{label}] {chap}:{sec}  full={row['full_frac']:.3f}  "
                  f"09xxx={r9}  {'+ylt' if row['has_ylt'] else 'NO-ylt'}  "
...
def main():
    ...
    for arm in arms:
        conv_on = (arm == "B")
        print(f"\n  ── Arm {arm} ({'conventions ON' if conv_on else 'control OFF'}) ──")
    ...
    for arm in arms:
        ...
        print(f"  Arm {arm}: n={len(rows)}  full_frac={mean(rows,'full_frac'):.3f}  "
              f"09xxx_recall={r9s}")
    if "B" in results and "B0" in results and results["B"] and results["B0"]:
        d = mean(results["B"], "full_frac") - mean(results["B0"], "full_frac")
        print(f"\n  Δ(B − B0) full_frac = {d:+.4f}   "
              f"({'s10 conventions help' if d > 0 else 'no measured gain'})")
