# Survey6 Prompt v0.1 — Original Language Anchored SN Transfer
# Task: KJV plain + Original (Heb/Grk) + KJV+SN + SN:word dict + UNV plain → UNV+SN
# Ground truth: FHL UNV+SN
# Framing: 跨語言標注投射 (Cross-Lingual Annotation Projection) — bi-lingual
...
## Input Format (5 inputs, all same verse)
...
5. **UNV (plain)** — Chinese (和合本), no tags ← **this is what you annotate**

Your output: the UNV text with SN tags embedded at the correct positions.

## How to Use the SN:word Dictionary

The dictionary entries look like:
```
WH07225: בְּרֵאשִׁית
WH01254: בָּרָא
WH00430: אֱלֹהִים  ← wait, use normalized form below
```
...
### 1. Every KJV SN Must Be Embedded
Count the SN tags in KJV+SN. Your output must have AT LEAST that many.
UNV may need MORE (900x prefixes, different implicit markers). Missing = failure.
...
- Copy tags character-for-character from KJV+SN input
...
1. Count SN tags in output ≥ count in KJV+SN input
