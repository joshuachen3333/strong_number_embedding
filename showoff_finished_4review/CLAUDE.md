...
cd llm_direct_sn_unv2lcc && python generate_manifest.py && python generate_data_bundle.py && cd ..
...
All verse data is pre-bundled in `data_bundle.json` so the viewer works without access to `../llm_direct_sn_unv2lcc/output/`.

### Multi-brand bundle format

Output is organized by brand: `output/{brand}/{Book}/{Chapter}/{verse}.json`. The bundle groups data by brand:
...
Generated by: `cd llm_direct_sn_unv2lcc && python generate_manifest.py && python generate_data_bundle.py`

Supported brands: **claude** (opus, sonnet), **gemini** (3-pro, 3-flash, 2.5-pro, 2.5-flash), **codex** (5.4, 5.3, 5.2). Brand tabs appear in the viewer when multiple brands have data.
