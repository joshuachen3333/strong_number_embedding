...
Single-file showcase viewer for the Strong's Numbers AI transfer results (multi-version, multi-brand), with authenticated review system.

## How to Start

```bash
# 1. Regenerate data bundle (if output files changed)
cd llm_direct_sn_unv2notyet && python generate_manifest.py && python generate_data_bundle.py && cd ..
...
All verse data is pre-bundled in `data_bundle.json` so the viewer works without access to `../llm_direct_sn_unv2notyet/output/`.

### Multi-version/brand bundle format

Output is organized by version and brand: `output/{version}/{brand}/{Book}/{Chapter}/{verse}.json`. The bundle groups data by version then brand:
...
Generated by: `cd llm_direct_sn_unv2notyet && python generate_manifest.py && python generate_data_bundle.py`

Supported brands: **claude** (opus, sonnet), **gemini** (3-pro, 3-flash, 2.5-pro, 2.5-flash), **codex** (5.4, 5.3, 5.2). Version selector and brand tabs appear in the viewer when multiple versions/brands have data.

Each verse JSON contains version-specific fields:

| Field | Purpose |
|-------|---------|
| `unv_sn_reference` | UNV text with Strong's Numbers (left panel) |
| `{version}_sn` | Target version text with AI-inserted SN (e.g., `lcc_sn`, `rcuv2010_sn`) |
| `{version}_original` | Plain target text without SN (e.g., `lcc_original`) |
| `target_version` | Version code (e.g., `lcc`, `rcuv2010`) |
| `confidence` | 0.0–1.0 AI confidence score |
| `notes` | Array of AI reasoning strings |
| `brand` | AI brand (`claude`, `gemini`, `codex`) |
| `model` | LLM model used (e.g., `sonnet`, `opus`, `gemini-3-pro`) |
...
├─────────────────────────────────────────────────────────┤
│ controls: version-select | brand-tabs | book | chap       │
├───────────────────────────┬─────────────────────────────┤
│ left panel (50%)          │ right panel (50%)            │
│ UNV+SN reference          │ Target+SN AI output          │
