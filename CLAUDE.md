...
```bash
# Generate manifest + data bundle for showoff viewer (after new output files)
cd llm_direct_sn_unv2notyet
python3 generate_manifest.py && python3 generate_data_bundle.py
```

### `llm_direct_sn_unv2notyet/survey1_prompt_evolving/` — 3-Model Gold Standard (Python)

Establishes a consensus "gold standard" SN embedding using three top LLMs (opus, gemini-3-pro-preview, gpt-5.4) with live prompt evolution. Uses a 3-round consensus protocol: Round 1 (unanimous) → Round 2 (convergence + debate, 2/3 majority) → Round 3 (pick winner or identify collective error → auto-evolve prompt).

**Key architectural rule**: `build_gold_standard()` in `consensus.py` is the **sole authority** for all `resolved_at` judgments. The main loop only collects data — never directly saves gold standard. See `ARCHITECTURE_DECISIONS.md` before modifying resolution logic.

```bash
cd llm_direct_sn_unv2notyet/survey1_prompt_evolving

# Default: Gen 1-2, auto-detects latest prompt, skips cached
...
# Force re-run, round control
python3 run_gold_standard.py --force
python3 run_gold_standard.py --round1-only
python3 run_gold_standard.py --show-summary
```

**Prompt lives in `prompts/`** (versioned `v1.0.md`, `v1.1.md`, …). Prompt evolves automatically during the run when Round 3 judges identify collective errors. Model-specific patches (`v1.1.opus-patch-0.1.md`) are auto-generated by `judge.py` for unstable models. Gold standard JSON saved to `gold_standard/{Book}/{chap}/{sec}.json`.

### `llm_direct_sn_unv2notyet/survey2_fhl_sn_format_spec/` — FHL SN Format Reference

Authoritative textbook-level reference for all FHL Strong's Number notation. **Read-only** — documents what FHL produces, not what we generate. All components (parser, LLM transfer, viewer) must conform to formats described in `FHL_SN_FORMAT_REFERENCE.md`.

### `showoff_finished_4review/` — Showcase Viewer with Review System (HTML/JS)

Single-file viewer for reviewing AI-generated SN results across multiple versions (LCC, RCUV2010) and brands (claude, gemini, codex). Includes authenticated reviewer system with email OTP and review types (comment, suggestion, approval, needs_work).

```bash
# 1. Regenerate data bundle if output files changed
cd llm_direct_sn_unv2notyet && python3 generate_manifest.py && python3 generate_data_bundle.py && cd ..

# 2. Start server (port 8989)
./showoff_finished_4review/start.sh
# Open http://localhost:8989/showoff_finished_4review/

# Remote sharing
ngrok http 8989  # Free tier requires ngrok-skip-browser-warning:1 header (already applied)
```

**Auth config** (gitignored): `showoff_finished_4review/smtp_config.json` (Gmail App Password for OTP email), `showoff_finished_4review/reviewers.json` (pre-approved reviewer emails). Without `smtp_config.json`, OTP codes print to server console.

**MUST READ `showoff_finished_4review/CLAUDE.md` before modifying.** Contains auth flow, API endpoints, and data bundle format details.
