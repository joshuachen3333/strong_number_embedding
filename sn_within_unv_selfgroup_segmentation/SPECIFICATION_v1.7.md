# UNV+SN 分組規格 v1.7（完整獨立版）

> 本規格定義由含 Strong 編號、形態碼、隱性語素與 900x 擴展碼的 UNV 經文（FHL `qb.php`）出發，轉為穩定、可機器處理的語義分組。v1.7 整合 v1.6 所有內容，並新增**複合介系詞自動檢測與合併機制**，直接從 qp.php 的 `wform` 字段提取複合信息，無需維護獨立字典。

**狀態**：✅ 已實作並測試通過（2025-11-24）
...
```python
def detect_compound_from_qp(current_token, next_token, qp_data):
    """
    直接從 qp.php 的 wform 字段檢測複合介系詞
    無需維護獨立字典
    """
    if current_token.strong_number != "04480":  # מִן
        return None

    # 查找 next_token 在 qp.php 中的記錄
    next_qp_record = find_qp_record(next_token, qp_data)
    if not next_qp_record:
        return None

    wform = next_qp_record.get('wform', '')

    # 檢查 wform 是否包含「介系詞 מִן +」模式
    if '介系詞 מִן +' in wform:
        # 提取複合詞信息
        return {
            'type': 'prep_plus_prep',
            'first_sn': '04480',
            'second_sn': next_token.strong_number,
            'structure': wform,  # 例如：「介系詞 מִן + 介系詞 עַל」
            'is_compound': True
        }

    # 檢查 prep+noun 複合詞（可選）
    if config.get('detect_prep_plus_noun', True):
        if '介系詞 מִן +' in wform and '名詞' in wform:
            return {
                ...
                'structure': wform,
                'is_compound': True,
                'merge': config.get('merge_prep_plus_noun', False)
            }
...
**觸發條件**：
1. 當前 token 為 `<04480>` (מִן)
2. 下一個 token 的 qp.php `wform` 包含「介系詞 מִן +」
3. 兩個 token 之間沒有標點或其他插入內容

**合併動作（prep+prep）**：
1. 創建新的複合組 `G_compound`
2. `G_compound.core = ["04480", second_sn]`
3. `G_compound.type = "compound_preposition"`
4. `G_compound.structure = wform`（從 qp.php 提取）
5. 移除原始的兩個獨立 token
6. **不生成** `qb_qp_mismatch` 警告

**prep+noun 處理**：
- 檢測到但**不合併**（根據 `merge_prep_plus_noun: False` 配置）
- 記錄到 `output/compound_prep_plus_noun.txt`
...
def detect_and_merge_compounds(tokens, qp_data, profile):
    ...
    while i < len(tokens):
        ...
        next_qp = find_qp_record(next_token, qp_data)
        ...
        if '介系詞 מִן +' in wform and '介系詞' in wform:
            if profile.get('merge_prep_plus_prep', True):
                # 合併
                compound = create_compound_token(
                    current, next_token, wform, 'prep_plus_prep'
                )
        ...
        if '介系詞 מִן +' in wform and '名詞' in wform:
            if profile.get('log_compound_prep_noun', True):
                log_prep_noun_compound(current, next_token, wform)

            if profile.get('merge_prep_plus_noun', False):
                # 可選合併
                compound = create_compound_token(
                    current, next_token, wform, 'prep_plus_noun'
                )
                result.append(compound)
                i += 2
                continue
...
def log_prep_noun_compound(first, second, wform):
    """記錄 prep+noun 複合詞到專用日誌"""
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    verse_ref = get_current_verse_ref()
    log_entry = (
        f"[{timestamp}] {verse_ref} | prep_noun_compound | "
        f"Prep+noun compound detected: <{first.strong_number}><{second.strong_number}> = "
...
```python
def check_qb_qp_mismatch(strong_number, qp_data, compound_log):
    """
    檢查 qb/qp 不匹配，考慮複合詞情況
    """
...
### Q4: v1.7 與 v1.6 輸出兼容嗎？

**兩種模式**：
- 啟用複合詞檢測：輸出包含複合詞信息（v1.7 增強版）
...
3. **compound_prep_plus_noun.txt**: prep+noun 複合詞記錄（v1.7）
...
> 本規格完整獨立，無需參考其他版本文檔。所有 v1.5/v1.6 內容已整合，所有 v1.7 新增功能已包含。可直接用於實作與測試。
