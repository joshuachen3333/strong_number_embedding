...
> **概念基礎（v1.9 新增）**：本規格處理的是 FHL「parsing」（L1 形態分析）的下游輸出；我們的 SN 插入任務本質是 **Alignment（詞對詞對應，或 null）**。上位概念框架見
> [`../parsing/PARSING_FOUNDATIONS.md`](../parsing/PARSING_FOUNDATIONS.md)。

**狀態**：📝 規格定義完成（2026-07-10）

**v1.9 新增**：qp-enrichment 文件級增量 S1–S4（`lemma` 補註欄位、粗/細兩級 Parsing Code 術語、OT/NT `pro`/`wform` 不對稱說明、概念基礎文檔指標）。純附加（additive-only），不改變任何 token 分類、分組或合併行為；詳見 §11.1。
...
### 1.2.3 v1.9 文件級增量（qp-enrichment，S1–S4）

* **S1 — `lemma` 補註欄位**：新增 `lemma: string | null`，取自 qp.php 的 `orig`（字典原形）；annotation-only，預設 `null`，不影響分組/`morph`/`prefixes`（§5.2.1、§6.1）
* **S2 — 兩級 Parsing Code 術語**：明確定義粗顆粒（qb 行內 `<WTH8804>`/`<WTG5656>`，僅動詞核心變化）與細顆粒（qp `wform`，另含人稱/性/數）（§2.4）
* **S3 — OT/NT 欄位不對稱**：記錄 qp.php `pro`/`wform` 在舊約/新約的語義不對稱；標記現行 parsing 推斷為 OT-centric（§6.1.1）
* **S4 — 概念基礎指標**：指向 `../parsing/PARSING_FOUNDATIONS.md`（Alignment vs Parsing 框架）（文件頭、§12.1）
...
* 複合介系詞合併可通過配置開關控制
* 可向後兼容 v1.6 行為
* v1.9 為純文件級附加：任何 v1.8 合規輸出即 v1.9 合規輸出（`lemma` 缺省視為 `null`）
...
### 2.4 Parsing Code 的兩級顆粒度（v1.9 新增）

同一份「Parsing Code」（詞形變化資訊）在 FHL 資料中以兩種顆粒度出現：

| 級別 | 來源 | 形式 | 內容 |
|------|------|------|------|
| **粗顆粒（coarse）** | `qb.php?strong=1` 行內標記 | `<WTH8804>` / `<WTG5656>`（即本規格的 morph 8xxx token） | 僅動詞核心變化（希伯來：語幹+時態；希臘：時態+語態+語氣） |
| **細顆粒（fine）** | `qp.php` 的 `wform` 欄位 | 中文描述字串 | 粗顆粒之上另含人稱/性/數（person / gender / number） |

範例（創 1:1 בָּרָא）：
- 粗：`<WTH8804>` ⇒ `(**8804)` = Qal 完成式
- 細：`wform` = 「動詞，Qal 完成式 3 單陽」（多出「3 單陽」）

範例（約 3:16 ἠγάπησεν）：
- 粗：`<WTG5656>` = 第一簡單過去 主動 直說語氣
- 細：`wform` = 「第一簡單過去 主動 直說語氣 第三人稱 單數」（即傳統代碼 V-AAI-3S）

**與本規格的關係**：morph token（8xxx）即粗顆粒；`parsing_wform` 補註（§6.1）即細顆粒。粗顆粒解碼表見 `../llm_direct_sn_unv2notyet/survey2_fhl_sn_format_spec/FHL_SN_FORMAT_REFERENCE.md` §6。兩級都是**同一個詞的變化資訊**，永不對應獨立的中文詞（見 §2.3、§3.3.2）。
...
* `lemma: string | null`（v1.9 新增；字典原形，取自 `qp.orig`；**僅補註（annotation-only）**，預設 `null`；不影響分組、`morph`、`prefixes` 或任何合併決策）
...
* **Lemma 補註（v1.9 新增）**：若可取得 qp 資料，將該詞 qp 記錄的 `orig`（字典原形）寫入 `lemma`；查無對應記錄、或未啟用 qp 輔助時保持 `null`。與 `parsing_wform` 同級：僅補註，嚴禁參與分組決策或 token 分類

#### 6.1.1 OT/NT `pro`/`wform` 欄位不對稱（v1.9 新增）

qp.php 的欄位語義在舊約與新約**不對稱**（已實測驗證；另見 wid=0 為全節總覽列，處理時應跳過）：

| | `pro` | `wform` |
|---|---|---|
| **OT（希伯來文）** | 空字串 | 詞類 + 全部詞形資訊（如「動詞，Qal 完成式 3 單陽」） |
| **NT（希臘文）** | 詞類（如「動詞」） | 僅屈折資訊（如「第一簡單過去 主動 直說語氣 第三人稱 單數」）；不變化詞為空字串 |

⚠️ **本規格現行的 parsing 推斷（§6.1 推斷前綴、§3.3 複合詞 `wform` 樣式比對如「介系詞 מִן +」）是 OT-centric 的**：它假設詞類與結構資訊出現在 `wform` 內。擴展到新約時必須改為先讀 `pro`（詞類）、再讀 `wform`（屈折），否則樣式比對會失敗。此不對稱另有結構性根源（希臘文屈折靠詞綴、希伯來文靠內部母音變化），見 `../parsing/PARSING_FOUNDATIONS.md` §4。
...
### 11.1 v1.9 vs v1.8 變更（2026-07-10）— qp-enrichment 文件級增量

**性質**：純附加（additive-only）。不改變任何 token 分類、分組、合併行為；v1.8 合規輸出即 v1.9 合規輸出。`SPECIFICATION_v1.8.md` 保持原樣不動（`parse_verse_v1_8.py` 啟動時仍對其做版本驗證）。OpenSpec 變更提案：`openspec/changes/add-qp-parsing-enrichment/`。

1. **S1 — `lemma` 補註欄位**（§5.2.1、§6.1）
   - 新增 `lemma: string | null`，取自 qp.php `orig`（字典原形）
   - annotation-only：預設 `null`，不影響分組/`morph`/`prefixes`
2. **S2 — 兩級 Parsing Code 術語**（§2.4）
   - 粗顆粒 = qb 行內 `<WTH8804>`/`<WTG5656>`（動詞核心變化）；細顆粒 = qp `wform`（另含 person/gender/number）
3. **S3 — OT/NT `pro`/`wform` 不對稱**（§6.1.1）
   - 記錄 OT `pro` 空 / NT 詞類在 `pro` 的實測差異；標記現行推斷為 OT-centric
4. **S4 — 概念基礎指標**（文件頭、§12.1）
   - 指向 `../parsing/PARSING_FOUNDATIONS.md`（Alignment vs Parsing 框架）
5. **編號修正**：原 §11 內重複的兩個「11.2」小節重新編號為 11.3/11.4（僅編號，內容不變）

### 11.2 v1.7.2 vs v1.7 變更（2025-11-25）
...
### 11.3 v1.7 vs v1.6 變更（2025-11-24）
...
- **概念基礎（v1.9 新增）**：`../parsing/PARSING_FOUNDATIONS.md`（Parsing vs Alignment 框架；本規格的上位概念文檔）
- **FHL 標記格式參考（v1.9 新增）**：`../llm_direct_sn_unv2notyet/survey2_fhl_sn_format_spec/FHL_SN_FORMAT_REFERENCE.md`（粗顆粒 parsing code 解碼表 §6；qp 欄位說明 §9.2）
