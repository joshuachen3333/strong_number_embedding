...
# This script implements SPECIFICATION_v1.7.2.md
...
def detect_min_compound(token_04480, tokens, current_index, qp_records):
    """
    Detect if <04480> (מִן) and following token(s) form a compound preposition.

    v1.7.2: This function now automatically skips intervening 900x prefixes
    to detect multi-token compounds like <04480><09001><06440> (מִלִּפְנֵי).

    This implements SPECIFICATION_v1.7.2.md compound detection by checking
    if the core token's qp.php wform indicates it contains מִן.

    Returns compound_info dict if detected (with 'should_merge' flag), None otherwise.
    """
    if not token_04480 or token_04480.get('value') != '04480':
        return None

    # v1.7.2: Skip over 900x prefixes to find the next core token
    j = current_index + 1
    intervening_900x = []

    while j < len(tokens) and tokens[j].get('type') == 'p900x':
        intervening_900x.append(tokens[j]['value'])
        j += 1

    if j >= len(tokens):
        # No core token found after 04480
        return None

    next_core_token = tokens[j]
    next_sn = next_core_token.get('value')
    next_sn_stripped = next_sn.lstrip('0')

    # v1.7.2: Find the qp record that contains מִן in wform
    # First try to find by matching SN
    qp_record = None
    for record in qp_records:
    ...
    # v1.7.2: If not found by SN, search all records for מִן pattern
    # This handles cases where qb.php splits tokens but qp.php uses different SN
    # Example: qb has <04480><09001><06440>, qp has single record with SN 03942
    if not qp_record:
        for record in qp_records:
            wform = record.get('wform', '')
            if '介系詞 מִן +' in wform:
                # Found a compound, use this record
                qp_record = record
                next_sn = record.get('sn', next_sn)  # Use qp's SN
                break
    ...
    # Check if wform contains מִן (indicating this is a compound)
    if 'מִן' not in wform:
        return None

    # Determine compound type from wform
    compound_type = None
    if '介系詞 מִן + 介系詞' in wform:
        compound_type = 'prep+prep'
    elif '介系詞 מִן + 名詞' in wform:
        compound_type = 'prep+noun'
    elif '介系詞 מִן + 專有名詞' in wform:
        compound_type = 'prep+proper_noun'
    elif '介系詞 מִן +' in wform:
        compound_type = 'prep+other'
    else:
        return None  # Contains מִן but not in expected pattern

    # Check if we should merge based on configuration
    should_merge = False
    ...
    # v1.7.2: Build components list including all involved tokens
    components = ['04480'] + intervening_900x + [next_sn]

    # Return compound information (even if not merging, for logging purposes)
    return {
        'type': compound_type,
        ...
        'intervening_900x': intervening_900x,  # Track skipped prefixes
        'tokens_to_skip': j - current_index  # How many tokens to skip in main loop
    }
...
def group_and_merge(tokens, qp_records):
    ...
    # --- Pass 0: Compound Detection (v1.7 new, v1.7.2 enhanced) ---
    # Detect and mark מִן (04480) compound prepositions
    if PROFILE['detect_compounds_from_qp']:
        i = 0
        while i < len(tokens):
            token = tokens[i]
            if token['value'] == '04480' and token['type'] in ['strong', 'implicit_strong']:
                # v1.7.2: Check if this forms a compound (may skip 900x prefixes)
                compound_info = detect_min_compound(token, tokens, i, qp_records)
                if compound_info:
                    if compound_info['should_merge']:
                        ...
                        # v1.7.2: Mark all tokens in the compound (including 900x)
