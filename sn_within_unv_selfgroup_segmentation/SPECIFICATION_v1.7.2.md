# UNV+SN 分組規格 v1.7.2（完整獨立版）

> 本規格定義由含 Strong 編號、形態碼、隱性語素與 900x 擴展碼的 UNV 經文（FHL `qb.php`）出發，轉為穩定、可機器處理的語義分組。v1.7.2 整合 v1.6 所有內容，並新增**複合介系詞自動檢測與合併機制**（含多 token 複合詞支持），直接從 qp.php 的 `wform` 字段提取複合信息，無需維護獨立字典。

**狀態**：✅ 已實作並測試通過（2025-11-25）

**v1.7.2 新增**：支持跨越 900x 前綴的複合介系詞檢測（如 `<04480><09001><06440>` = מִלִּפְנֵי "from before"）
...
#### 3.3.1 檢測算法（v1.7.2 增強：支持多 token）

```python
def detect_compound_from_qp(tokens, current_index, qp_data, config):
    """
    v1.7.2: 直接從 qp.php 的 wform 字段檢測複合介系詞
    支持跨越 900x 前綴的多 token 複合詞
    無需維護獨立字典
    """
    ...
    # v1.7.2 新增：跳過 900x 前綴，找到下一個 core token
    j = current_index + 1
    intervening_900x = []
    ...
    next_qp_record = find_qp_record(next_core_token, qp_data)
    ...
    if '介系詞 מִן +' in wform:
        # v1.7.2: 記錄所有相關 tokens（包括中間的 900x）
        involved_tokens = [current_token] + intervening_900x + [next_core_token]
        token_count = len(involved_tokens)

        if '介系詞' in wform:
            # prep+prep 複合詞
            return {
                'type': 'prep_plus_prep' if token_count == 2 else 'prep_plus_prep_complex',
                'involved_tokens': involved_tokens,
                'end_index': j,  # 下一個要處理的索引
                'structure': wform,
                'is_compound': True
            }
        elif '名詞' in wform:
            # prep+noun 複合詞
            return {
                'type': 'prep_plus_noun' if token_count == 2 else 'prep_plus_noun_complex',
                'involved_tokens': involved_tokens,
                'end_index': j,
                'structure': wform,
                'is_compound': True,
...
#### 3.3.2 合併規則（v1.7.2 增強）

**觸發條件**：
1. 當前 token 為 `<04480>` (מִן)
2. v1.7.2: 跳過任何中間的 900x 前綴
3. 下一個 core token 的 qp.php `wform` 包含「介系詞 מִן +」
4. tokens 之間沒有標點或非 900x 插入內容

**合併動作（prep+prep）**：
1. 創建新的複合組 `G_compound`
2. v1.7.2: `G_compound.core` 包含所有相關 tokens（含中間 900x）
3. `G_compound.type = "compound_preposition"` 或 `"compound_preposition_complex"`
...
- v1.7.2: 記錄所有相關 tokens 到 `output/compound_prep_plus_noun.txt`
- **不記錄**到 `uncertain_or_expandable_issues.txt`（避免重複）
- 這些是 FHL 數據編碼特性，非解析錯誤

**v1.7.2 範例**：
- 簡單: `<04480><05921>` → מֵעַל (2 tokens)
- 複雜: `<04480><09001><06440>` → מִלִּפְנֵי (3 tokens, 含 900x)
