#!/bin/bash
# Batch parse Genesis, Exodus, and Leviticus
set -e
...
for chap in {1..50}; do
    ...
    for verse in $(seq 1 $max); do
        python3 run_parser_temp.py --book Gen $chap $verse > /dev/null 2>&1
    done
done
...
for chap in {1..40}; do
    ...
    for verse in $(seq 1 $max); do
        python3 run_parser_temp.py --book Exod $chap $verse > /dev/null 2>&1
    done
done
...
for chap in {1..27}; do
    ...
    for verse in $(seq 1 $max); do
        python3 run_parser_temp.py --book Lev $chap $verse > /dev/null 2>&1
    done
done
...
echo ""
echo "Summary:"
echo "  Genesis: 50 chapters parsed"
echo "  Exodus: 40 chapters parsed"
echo "  Leviticus: 27 chapters parsed"
echo ""
